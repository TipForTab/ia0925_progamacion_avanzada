...
class PropertyExtractor:
    ...
    def _extract_property_type(self) -> Optional[str]:
        ...
        if typology:
            ...
            if any(
                word in type_text
                for word in ["piso", "apartamento", "ático", "dúplex", "estudio"]
            ):
                return "apartment"
            ...
            if any(
                word in type_text
                for word in ["casa", "chalet", "villa", "adosado", "unifamiliar"]
            ):
                return "house"
...
class BatchExtractor:
    ...
    def process_all(self, verbose: bool = True) -> Dict:
        ...
        for i, html_file in enumerate(html_files, 1):
            if verbose:
                print(
                    f"[{i}/{len(html_files)}] Processing: {html_file.name}...", end=" "
                )
        ...
        if verbose:
            print("=" * 60)
            print(
                f"Completed: {len(self.results)} successful, {len(self.errors)} errors"
            )
