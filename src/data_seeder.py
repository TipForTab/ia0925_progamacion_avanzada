...
DEFAULT_USERS = [
    {"username": "antonio", "email": "antonio@example.com", "password": "admin*2026"},
    {
        "username": "daniellab",
        "email": "daniellab@example.com",
        "password": "admin*2026",
    },
    {"username": "danielgo", "email": "danielgo@example.com", "password": "admin*2026"},
]
...
class PropertyParser:
    ...
    def parse(self, raw_data: Dict) -> Optional[ParsedPropertyData]:
        ...
        bathrooms = parsed_features.get("bathrooms") or self.estimate_bathrooms(
            rooms, features
        )
...
class UserSeeder:
    ...
    async def create_user(
        self, session, username: str, email: str, password: str
    ) -> User:
        ...
        async with self.get_session() as session:
            try:
                for user_data in self.users:
                    ...
                    print(
                        f"✅ User '{username}': Created (email: {user_data['email']})"
                    )
...
class PropertyImporter:
    ...
    def __init__(self, json_file_path: str = JSON_FILE_PATH):
        ...
        self.stats = {"imported": 0, "skipped": 0, "errors": 0, "total_images": 0}
    ...
    def load_json(self) -> List[Dict]:
        ...
        if isinstance(data, dict) and "properties" in data:
            ...
            if metadata.get("extraction_errors"):
                print(
                    f"⚠️  Warning: {metadata['extraction_errors']} extraction errors in source"
                )
        else:
            raise ValueError(
                "Invalid JSON format. Expected {'properties': [...], 'metadata': {...}}"
            )
    ...
    async def save_images(
        self, session, property_id: int, image_urls: List[str]
    ) -> int:
        ...
        for img_url in image_urls:
            try:
                result = await session.execute(
                    select(Image).where(
                        Image.property_id == property_id, Image.img_url == img_url
                    )
                )
                ...
                image = Image(
                    property_id=property_id, img_url=img_url, calculated_tags=None
                )
                session.add(image)
                saved_count += 1
        ...
        try:
            ...
            if parsed_data.image_urls:
                images_saved = await self.save_images(
                    session, db_property.id, parsed_data.image_urls
                )
                self.stats["total_images"] += images_saved

            property_type_str = "Apartment" if parsed_data.is_apartment else "House"
            operation = (
                parsed_data.amenities.get("operation_type", "unknown")
                if parsed_data.amenities
                else "unknown"
            )
            print(
